...
const ArticleCard = ({ content, uiType, mdSize = 'col-md-4', className }: ArticleCardProps) => {
  ...
  const contentTypeNm = isMentorSeminarType ? `${content?.seminarPlaceTypeName} 세미나` : content.contentsTypeName;
  const articleData = content?.registDate ? content?.registDate : content?.createdAt;
  const seminarStartData = content?.seminarStartDate ? content?.seminarStartDate : content?.createdAt;
  const seminarEndDateText = isMentorSeminarType ? content.seminarEndDate.split(' ')[0].replace(/-/gi, '.') : null;
  let imageUrl = isMentorSeminarType ? content.imageUrl2 : content.imageUrl;

  //세미나 마감 로직
  const deadlineCount = Math.floor(content.participantCount / 2) | 0;
  const currentParticipantCount = content.currentParticipantCount;
          ...
          <div className={cx('img-area')}>
              ...
              <Image
                src={
                  imageUrl?.indexOf('http') > -1
                    ? imageUrl
                    : `${process.env['NEXT_PUBLIC_GENERAL_IMAGE_URL']}/images/${imageUrl}`
                }
            ...
            <div className={cx('img-area__chip')}>
              <Chip chipColor="primary" radius={4} style={{ zIndex: 999999 }}>
                {content.recommendLevels.length === 5 ? '모든' : content.recommendLevels.sort().join(',')}레벨
              </Chip>

              {content.seminarStatus == '0002' && deadlineCount <= currentParticipantCount ? (
