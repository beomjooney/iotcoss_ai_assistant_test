...
const KnowledgeItem = ({ title, date, content, tags, likes, comments, hashtags }) => {
  return (
    <div className="tw-flex tw-flex-col tw-justify-start tw-items-start tw-w-[1120px] tw-relative tw-gap-2">
      <div className="tw-flex-grow-0 tw-flex-shrink-0 tw-w-[1120px] tw-h-20 tw-relative tw-overflow-hidden tw-rounded-lg tw-bg-[#f6f7fb]">
        <p className="tw-w-[953px] tw-absolute tw-left-7 tw-top-7 tw-text-base tw-text-left tw-text-black">{title}</p>
        <div className="tw-flex tw-justify-end tw-items-center tw-absolute tw-left-[1004px] tw-top-[26px] tw-gap-2">
          <p className="tw-flex-grow-0 tw-flex-shrink-0 tw-text-xs tw-text-right tw-text-[#9ca5b2]">{date}</p>
          <svg
            width={28}
            height={28}
            viewBox="0 0 28 28"
            ...
            className="tw-flex-grow-0 tw-flex-shrink-0 tw-w-7 tw-h-7 tw-relative"
      ...
      <div className="tw-flex-grow-0 tw-flex-shrink-0 tw-w-[1120px] tw-h-[146px] tw-relative tw-overflow-hidden tw-rounded-lg tw-bg-white tw-border tw-border-[#e9ecf2]">
        <svg
          width={24}
          height={24}
          viewBox="0 0 24 24"
        ...
        <p className="tw-absolute tw-left-[52px] tw-top-[58px] tw-text-sm tw-font-medium tw-text-left tw-text-[#31343d]">
          모범답안 :
        </p>
        <p className="tw-w-[973px] tw-absolute tw-left-[119px] tw-top-[58px] tw-text-sm tw-text-left tw-text-[#31343d]">
          {content}
        </p>
        <div className="tw-flex tw-justify-start tw-items-center tw-absolute tw-left-[52px] tw-top-5 tw-gap-2">
          {/* Render tags */}
          {tags.map((tag, index) => (
            <div
              key={index}
        ...
        <div className="tw-flex tw-justify-start tw-items-center tw-absolute tw-left-[1004px] tw-top-[109px] tw-gap-2">
          <div className="tw-flex-grow-0 tw-flex-shrink-0 tw-w-10 tw-h-[21px]">
            <div className="tw-flex tw-justify-start tw-items-center tw-absolute tw-left-0 tw-top-0 tw-gap-1">
              ...
              <p className="tw-flex-grow-0 tw-flex-shrink-0 tw-text-sm tw-text-left tw-text-[#9ca5b2]">{likes}</p>
            </div>
          </div>
          <div className="tw-flex tw-justify-start tw-items-center tw-flex-grow-0 tw-flex-shrink-0 tw-relative tw-gap-1">
            ...
            <p className="tw-flex-grow-0 tw-flex-shrink-0 tw-text-sm tw-text-left tw-text-[#9ca5b2]">{comments}</p>
          </div>
        </div>
        <div className="tw-flex tw-justify-start tw-items-start tw-absolute tw-left-[52px] tw-top-28 tw-gap-3">
          ...
          <p className="tw-flex-grow-0 tw-flex-shrink-0 tw-text-xs tw-font-medium tw-text-left tw-text-[#9ca5b2]">
            [아티클] CircuitBreaker를 이용한 외부 API 장애 관리
          </p>
        </div>
      </div>
    </div>
...
const KnowledgeComponent = ({ knowledgeData }) => {
  return (
    <div>
      {knowledgeData.map((data, index) => (
        <KnowledgeItem key={index} {...data} />
      ))}
    </div>
  );
};
