...
const KnowledgeComponent = ({ contents }) => {
    ...
    <div>
      {contents?.map((data, index) => (
        <div key={index} className="tw-pb-6">
          ...
          <div className="tw-p-5 tw-mt-3 tw-flex-grow-0 tw-flex-shrink-0  tw-h-[146px] tw-relative tw-overflow-hidden tw-rounded-lg tw-bg-white border">
            <div className="tw-pb-5 tw-flex tw-justify-start tw-items-center text-sm tw-left-[52px] tw-top-5 tw-gap-2">
              ...
              {data.skills?.map((hashtag, hashtagIndex) => (
                ...
                >
                  <p className="tw-flex-grow-0 tw-flex-shrink-0 tw-text-sm tw-text-left tw-text-[#ced4de]">
                    #{hashtag}
                  </p>
                </div>
              ))}
