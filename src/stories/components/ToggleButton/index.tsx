...
function ToggleButtonRoot(props: ToggleProps, ref: ForwardedRef<HTMLInputElement>) {
  const {
    type = 'default',
    register,
    required = false,
  ...
  return (
    <label
      className={cx('container', classType, className, 'tw-mr-3')}
    ...
    >
      ...
      >
        {isValidElement(props.label) ? (
          ...
          <Typography type={type === 'tabButton' || type === 'tabRadio' ? 'B2' : 'C1'} weight={weight}>
            {props.label}
          </Typography>
        )}
...
export default forwardRef<HTMLInputElement, ToggleProps>(ToggleButtonRoot);
