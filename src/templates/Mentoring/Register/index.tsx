...
interface MentoringRegisterTemplateProps {
  experiences?: ExperiencesResponse;
  skills?: SkillResponse;
  jobs?: any[];
  onGetJobsData?: () => void;
  onMentorSubmit?: (params: any) => void;
  type?: 'MENTOR' | 'MENTEE' | string;
  userType: any; // 0001 멘티
}

const cx = classNames.bind(styles);

export function MentoringRegisterTemplate({
  experiences,
  skills,
  jobs,
  onGetJobsData,
  ...
  type,
  userType,
}: MentoringRegisterTemplateProps) {
  ...
  const onSubmit = (data: any) => {
    ...
    if (params.skillIds.length === 0) {
      alert('멘토님께서 현재 보유하신 스킬을 선택해주세요.');
      return;
    }
    if (params.experienceIds.length === 0) {
      alert('멘토님께서 지금까지 보유하신 경험을 선택해주세요.');
      return;
    }
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit, onError)}>
      ...
      <div className={cx('mentoring-register-container', 'container pl-5 pr-5 pt-100')}>
        ...
        <article className="pt-100">
          <SectionHeader title="멘토님께서 현재 보유하신 스킬을 선택해주세요." subTitle="CHECK 04" />
          <div className={cx('filter-area', 'row pb-5')}>
        ...
        <article className="pt-100">
          <SectionHeader title="멘토님께서 지금까지 보유하신 경험을 선택해주세요." subTitle="CHECK 05" />
          <div className={cx('filter-area', 'row pb-5')}>
            <div className={cx('skill__group', 'col-md-12')}>
        ...
        <div className="col-md-6 pt-100" style={{ display: 'block', margin: 'auto' }}>
          <Button
            color="primary"
            label={userType === '0001' && type === 'MENTOR' ? '멘토 등록 신청하기' : '성장스토리 입력하기'}
            size="large"
            type="submit"
          />
        </div>
...
export default MentoringRegisterTemplate;
