...
/**import quiz modal  */
import { useQuizList, useGetQuizSchedule } from 'src/services/jobs/jobs.queries';
...
//group
import { dayGroup, openGroup, images, feedbackGroup, imageBanner } from './group';
...
export function ManageQuizClubTemplate({ id, title, subtitle }: ManageQuizClubTemplateProps) {
  ...
  const { mutate: onCrewRejectAll, isSuccess: isRejectAllSuccess } = useCrewRejectAllPost();
  ...
  const { refetch: refetchGetTemp }: UseQueryResult<any> = useClubAboutDetailInfo(ids, data => {
    ...
    const transformQuizData = quizData => {
      return quizData.map(quiz => ({
        ...quiz,
        quizSequence: quiz.quizSequence < 0 ? null : quiz.quizSequence,
      }));
    };

    setScheduleQuizData(quizListData);

    const transformedQuizData = transformQuizData(quizListData);
    console.log('transformedQuizData', transformedQuizData);
  ...
  const [scheduleQuizData, setScheduleQuizData] = useState<any[]>([]);
  ...
  }, [isAcceptSuccess, isRejectSuccess, isAcceptAllSuccess, isRejectAllSuccess]);
  ...
  function handleJoinMemberApprove(): void {
  ...
  function handleJoinMemberReject(): void {
    const isConfirmed = window.confirm('클럽 회원을 일괄 거절하시겠습니까?');
    if (isConfirmed) {
      console.log(memberUUID);
      let params = {
        club: selectedClub?.clubSequence,
      };
      onCrewRejectAll(params);
      // 회원 강퇴 로직 추가
    }
  }
  ...
  const handleInputBlur = (index, part) => {
    const updatedScheduleData = [...scheduleQuizData];
  ...
  function renderDatesAndSessionsModify() {
    return (
      <div className="tw-grid tw-grid-cols-12 tw-gap-4 tw-p-3 tw-mt-4">
        {scheduleQuizData.map((session, index) => (
  ...
  const handlerClubSaveTemp = () => {
    ...
    scheduleQuizData.forEach((item, i) => {
              ...
              <div className={cx('content-wrap')}>
                <div className={cx('container', 'tw-mt-10')}>
                  ...
                  {myMemberRequestList.length > 0 && (
                    <div className="tw-flex tw-justify-end tw-gap-3 tw-my-5">
                      <button
                        className="tw-text-sm tw-text-right tw-bg-black tw-text-white tw-px-5 tw-py-2 tw-rounded"
                        onClick={() => handleJoinMemberApprove()}
                      >
                        일괄 승인
                      </button>
                      <button
                        className="tw-text-sm tw-text-right tw-bg-black tw-text-white tw-px-5 tw-py-2 tw-rounded"
                        onClick={() => handleJoinMemberReject()}
                      >
                        일괄 거절
                      </button>
                    </div>
                      ...
                      return (
                        ...
                        >
                          <div className="tw-pt-3">
                            ...
                            ) : quizType === '0200' && item?.clubQuizType === '0001' ? (
                              <div
