...
interface CompanyLoginTemplateProps {
  title: string;
  onSubmitLogin: () => void;
}

export function CompanyLoginTemplate({ title = '', onSubmitLogin }: CompanyLoginTemplateProps) {
  ...
  const renderLogin0100 = () => (
    <div>
      <div className={cx('logo-area')}>
        ...
        <p className={cx('tw-font-bold tw-text-[26px] tw-text-black tw-py-5 tw-text-center')}>
          {title} 기업회원 로그인
        </p>
      </div>
  ...
  const renderLogin0200Only = () => (
    <div className="tw-mt-20">
      <div className="tw-w-full tw-max-w-md tw-mx-auto">
        <div className="mt-4">
          <div className={cx('logo-area')}>
            <p className={cx('tw-font-bold tw-text-[26px] tw-text-black tw-py-5 tw-text-center')}>기업회원 로그인</p>
          </div>
          <form onSubmit={handleSubmit(onSubmit0002, onError)}>
            <Typography sx={{ fontSize: 14, marginTop: 3, color: 'black', fontWeight: '600' }}>이메일</Typography>
            <TextField
          ...
          <Box display="flex" justifyContent="center" sx={{ fontWeight: 'bold' }}>
            <Typography
              onClick={() => router.push('/account/company-signup')}
              sx={{ fontSize: 14, textDecoration: 'underline' }}
              className="tw-py-3  tw-cursor-pointer"
            >
              회원가입
...
export default CompanyLoginTemplate;
